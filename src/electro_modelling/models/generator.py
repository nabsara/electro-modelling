...
class Generator(nn.Module):
    ...
    def __init__(self, dataset, z_dim, img_chan, hidden_dim, nmel_ratio=0):
        ...
        if dataset == "MNIST":
            ...
            self.model = nn.Sequential(
                self.make_gen_block(
                    input_channels=z_dim, output_channels=256, kernel_size=3, stride=2
                ),
                self.make_gen_block(
            ...
            self.model = nn.Sequential(
                nn.ConvTranspose2d(
                    z_dim, hidden_dim * 8, (2 * nmel_ratio, 2), stride=1
                ),
                self.make_gen_block_techno(
                    input_channels=hidden_dim * 8,
                    output_channels=hidden_dim * 8,
                    kernel_size=(3, 3),
                    stride=1,
                ),
    ...
    def make_gen_block_techno(
        self, input_channels, output_channels, kernel_size, stride, padding="same"
    ):
        """
        Build the sequence of operations corresponding to a generator block
        of DCGAN :
        - transposed convolution
        - batchnorm
        - ReLU activation
