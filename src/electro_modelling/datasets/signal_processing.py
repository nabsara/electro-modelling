...
class SignalOperators:
    ...
    def backward(self,STFT_mel):
        if self.phase_rec_method == 'Griffin_Lim': # Use the Griffin-Lim algorithm to reconstruct the phase from a STFT amplitude
    ...
    def griffin_phase(self,STFT_amp):
        ...
        if phase_init is None:
            STFT_hat = STFT_amp*np.exp( 1j*np.random.rand(self.nfft2, self.nb_trames)*np.pi/2)
        else:
